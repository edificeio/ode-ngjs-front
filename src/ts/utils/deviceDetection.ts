...
export function detectDevice(): DeviceInfo {
  ...
  const deviceVendor = result.getDevice().vendor || '';
  const deviceModel = result.getDevice().model || '';
  const deviceName =
    deviceVendor && deviceModel
      ? `${deviceVendor} ${deviceModel}`
      : deviceModel || deviceVendor || 'Unknown';

  let osName = result.getOS().name || 'Unknown';
  let deviceType = result.getDevice().type || (isPadDevice ? 'tablet' : 'desktop');
  ...
  return {
    osName,
    osVersion: result.getOS().version || 'Unknown',
    deviceType,
